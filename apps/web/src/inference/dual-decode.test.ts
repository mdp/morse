...
describe('findInterCallsignSplit', () => {
  ...
  it('ignores brief intra-character gaps that are too short', () => {
    ...
    const env = makeFakeEnvelope(T, [
      [100, 200],
      [220, 400], // first callsign with a 20-frame intra gap
      [600, 750],
      [770, 900], // second callsign with a 20-frame intra gap
    ]);
    const split = findInterCallsignSplit(env);
...
describe('alignAndMergeDecodes', () => {
  ...
  it('returns the non-empty side when one is empty', () => {
    expect(alignAndMergeDecodes(make('', 0), make('K1ABC', 0.5)).text).toBe(
      'K1ABC'
    );
    expect(alignAndMergeDecodes(make('K1ABC', 0.5), make('', 0)).text).toBe(
      'K1ABC'
    );
  });
  ...
  it('on substitution, picks the side with higher overall confidence', () => {
    expect(
      alignAndMergeDecodes(make('K1ABZ', 0.8), make('K1ABC', 0.5)).text
    ).toBe('K1ABZ');
    expect(
      alignAndMergeDecodes(make('K1ABZ', 0.5), make('K1ABC', 0.8)).text
    ).toBe('K1ABC');
  });
});
