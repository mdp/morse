...
async function loadCallsigns(basePath: string): Promise<string[]> {
  ...
  if (calls.length < 1000) {
    throw new Error(
      `Only loaded ${calls.length} callsigns - database may be corrupt`
    );
  }
 ...
 */
export function initCallsignDatabase(
  basePath: string = '.'
): Promise<string[]> {
  if (loadedCallsigns) {
...
export function getCallsigns(): string[] {
  if (!loadedCallsigns) {
    throw new Error(
      'Callsign database not loaded - call initCallsignDatabase() first'
    );
  }
...
export function getRandomCallsigns(count: number): string[] {
  ...
  const rotated = [
    ...shuffled.slice(randomStart),
    ...shuffled.slice(0, randomStart),
  ];
