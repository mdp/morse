...
import type { Caller, ContestState, OperatorState } from '../types';
import {
  createCaller,
  getCallerReply,
  getCallerReplyForCwt,
  processMessage,
...
export function useQsoFlow({
  ...
  const pendingRepliesRef = useRef<Map<string, ReturnType<typeof setTimeout>>>(
    new Map()
  );
  ...
  const generatePileup = useCallback((): Caller[] => {
    ...
    if (state.contestType === 'cwt') {
      ...
      const availableMembers = cwopsPool.filter(
        (m) => !workedCallsSet.has(m.call)
      );
      ...
      for (let i = 0; i < Math.min(count, shuffledMembers.length); i++) {
        ...
        callers.push(
          createCaller(entry, state.wpm, state.contestSettings, cwtData)
        );
      }
    ...
    return shuffle(availableCalls)
      .slice(0, count)
      .map((entry) => createCaller(entry, state.wpm, state.contestSettings));
  ...
  const scheduleCallerReply = useCallback(
    (caller: Caller, text: string, additionalDelay: number = 0) => {
  ...
  const findMatchingCaller = useCallback(
    (partial: string): Caller | null => {
  ...
  const sendExchange = useCallback(
    async (caller?: Caller) => {
      const targetCaller =
        caller || state.selectedCaller || findMatchingCaller(state.currentCall);
      ...
      if (
        ...
        state.selectedCaller.id !== targetCaller.id
      ) {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, 200 + targetCaller.sendDelay)
      );
  ...
  const logAndTu = useCallback(
    async (withCq: boolean = true) => {
  ...
  const handleF5 = useCallback(async () => {
    if (!audio.isRunning || audio.isSending || !state.currentCall.trim())
      return;
