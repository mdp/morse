...
import {
  extractWpxPrefix,
  getContinentFromPrefix,
} from '../utils/prefixExtractor';
  ...
  switch (action.type) {
    ...
    case 'LOG_QSO': {
      ...
      if (state.contestType === 'cwt') {
        ...
        const isBustedName = !validateCwtName(
          state.enteredName,
          cwtExchange.name
        );
        const isBustedNumber = !validateCwtExchange(
          state.enteredNumber,
          cwtExchange.number,
          cwtExchange.isMember
        );
      ...
      const myContinent = getContinentFromPrefix(
        extractWpxPrefix(state.userCall)
      );
...
export function useContestState() {
  ...
  const setQsoState = useCallback(
    (qsoState: QsoState) =>
      dispatch({ type: 'SET_QSO_STATE', state: qsoState }),
    []
  );
  const setCurrentCall = useCallback(
    (call: string) => dispatch({ type: 'SET_CURRENT_CALL', call }),
    []
  );
  const selectCaller = useCallback(
    (caller: Caller) => dispatch({ type: 'SELECT_CALLER', caller }),
    []
  );
  ...
  const setEnteredRst = useCallback(
    (rst: string) => dispatch({ type: 'SET_ENTERED_RST', rst }),
    []
  );
  const setEnteredNr = useCallback(
    (nr: string) => dispatch({ type: 'SET_ENTERED_NR', nr }),
    []
  );
  const addCallers = useCallback(
    (callers: Caller[]) => dispatch({ type: 'ADD_CALLERS', callers }),
    []
  );
  const clearCallers = useCallback(
    () => dispatch({ type: 'CLEAR_CALLERS' }),
    []
  );
  const markCallerHeard = useCallback(
    (id: string) => dispatch({ type: 'MARK_CALLER_HEARD', id }),
    []
  );
  const logQso = useCallback(() => dispatch({ type: 'LOG_QSO' }), []);
  const setUserCall = useCallback(
    (call: string) => dispatch({ type: 'SET_USER_CALL', call }),
    []
  );
  const setWpm = useCallback(
    (wpm: number) => dispatch({ type: 'SET_WPM', wpm }),
    []
  );
  const setQrnEnabled = useCallback(
    (enabled: boolean) => dispatch({ type: 'SET_QRN_ENABLED', enabled }),
    []
  );
  const setSnr = useCallback(
    (snr: number) => dispatch({ type: 'SET_SNR', snr }),
    []
  );
  const setBandwidth = useCallback(
    (bandwidth: number) => dispatch({ type: 'SET_BANDWIDTH', bandwidth }),
    []
  );
  const updateCaller = useCallback(
    (id: string, updates: Partial<Caller>) =>
      dispatch({ type: 'UPDATE_CALLER', id, updates }),
    []
  );
  const removeCaller = useCallback(
    (id: string) => dispatch({ type: 'REMOVE_CALLER', id }),
    []
  );
  const setActivity = useCallback(
    (activity: number) => dispatch({ type: 'SET_ACTIVITY', activity }),
    []
  );
  const setQsbEnabled = useCallback(
    (enabled: boolean) => dispatch({ type: 'SET_QSB_ENABLED', enabled }),
    []
  );
  const setQrmEnabled = useCallback(
    (enabled: boolean) => dispatch({ type: 'SET_QRM_ENABLED', enabled }),
    []
  );
  const setFlutterEnabled = useCallback(
    (enabled: boolean) => dispatch({ type: 'SET_FLUTTER_ENABLED', enabled }),
    []
  );
  const setLidsEnabled = useCallback(
    (enabled: boolean) => dispatch({ type: 'SET_LIDS_ENABLED', enabled }),
    []
  );

  // CWT-specific action creators
  const setContestType = useCallback(
    (contestType: ContestType) =>
      dispatch({ type: 'SET_CONTEST_TYPE', contestType }),
    []
  );
  const setMyName = useCallback(
    (name: string) => dispatch({ type: 'SET_MY_NAME', name }),
    []
  );
  const setMyNumber = useCallback(
    (number: string) => dispatch({ type: 'SET_MY_NUMBER', number }),
    []
  );
  const setEnteredName = useCallback(
    (name: string) => dispatch({ type: 'SET_ENTERED_NAME', name }),
    []
  );
  const setEnteredNumber = useCallback(
    (number: string) => dispatch({ type: 'SET_ENTERED_NUMBER', number }),
    []
  );
  ...
  const verifiedQsos = state.log.filter(
    (q) => !q.isDupe && !q.isBustedExchange
  ).length;
  const verifiedMultipliers = state.log.filter(
    (q) => q.isMultiplier && !q.isBustedExchange
  ).length;
  const verifiedPoints = state.log.reduce(
    (sum, q) => sum + q.verifiedPoints,
    0
  );
