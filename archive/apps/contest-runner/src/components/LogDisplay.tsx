import type { ContestType, QsoEntry } from '../types';
            ...
            {log.map((entry, index) => (
              <tr key={entry.id} className={getRowClass(entry)}>
                  ...
                  <>
                    <td className={entry.isBustedName ? 'busted-name' : ''}>
                      ...
                      {entry.isBustedName && entry.actualName && (
                        <span
                          className="correct-name"
                          title={`Should be ${entry.actualName}`}
                        >
                          ({entry.actualName})
                        </span>
                      )}
                    ...
                    <td className={entry.isBustedNumber ? 'busted-nr' : ''}>
                      ...
                      {entry.isBustedNumber && entry.actualNumber && (
                        <span
                          className="correct-nr"
                          title={`Should be ${entry.actualNumber}`}
                        >
                          ({entry.actualNumber})
                        </span>
                      )}
                  ...
                  <>
                    ...
                    <td className={entry.isBustedExchange ? 'busted-nr' : ''}>
                      ...
                      {entry.isBustedExchange && (
                        <span
                          className="correct-nr"
                          title={`Should be ${entry.actualSerial}`}
                        >
                          ({entry.actualSerial})
                        </span>
                      )}
                ...
                <td className="status-col">
                  {entry.isDupe && (
                    <span className="status-badge dupe">DUP</span>
                  )}
