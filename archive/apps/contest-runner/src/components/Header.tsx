      ...
      <div className="header-stats">
        ...
        <div className="stat-group">
          ...
          <div className="stat-row">
            ...
            <span
              className={`stat-value verified ${hasErrors ? 'has-errors' : ''}`}
            >
              {verifiedQsos}
            </span>
          </div>
        ...
        {hasErrors && (
          <div className="error-stats">
            {dupeCount > 0 && (
              <span className="error-stat dupe">{dupeCount} DUP</span>
            )}
            {contestType === 'cwt' ? (
              // CWT-specific error display
              <>
                {bustedNameCount > 0 && (
                  <span className="error-stat busted">
                    {bustedNameCount} NAME
                  </span>
                )}
                {bustedNumberCount > 0 && (
                  <span className="error-stat busted">
                    {bustedNumberCount} NR
                  </span>
                )}
              </>
            ) : (
              bustedCount > 0 && (
                <span className="error-stat busted">{bustedCount} NR?</span>
              )
            )}
          </div>
        )}
