 ...
 */
export function msgCallAndExchange(
  call: string,
  serial: number,
...
function formatCallerSerial(serial: number): string {
  ...
  // Apply cut numbers
  return padded.replace(/0/g, 'T').replace(/9/g, 'N');
}
 ...
 */
export function msgCwtExchange(
  name: string,
  number: string,
  makeError: boolean = false
): string {
 ...
 */
export function msgCwtCallAndExchange(
  call: string,
  name: string,
  number: string,
  makeError: boolean = false
 ...
 */
export function getCwtExchangeMessage(
  theirCall: string,
  myName: string,
  myNumber: string
): string {
  // Format the number with cut numbers if it's numeric
  const numberStr = /^\d+$/.test(myNumber)
    ? formatCwtNumber(myNumber)
    : myNumber;
...
function formatCwtNumber(number: string): string {
  return number.replace(/0/g, 'T').replace(/9/g, 'N');
}
