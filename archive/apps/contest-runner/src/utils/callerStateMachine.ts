...
import { type MatchResult, matchCall } from './callMatch';
 ...
 */
function processTimeout(caller: Caller): {
...
function formatCwtExchange(exchange: CwtExchange, makeError: boolean): string {
  ...
  if (isMember && /^\d+$/.test(number)) {
    number = number.replace(/0/g, 'T').replace(/9/g, 'N');
