...
import type { GameMode, HighScoreEntry, Settings } from '../types';
...
export function StartScreen({
  ...
  const filteredScores = highScores.filter(
    (h) => (h.mode || 'original') === settings.gameMode
  );
    ...
    <div className="start-screen">
      ...
      {dbStatus === 'ready' && (
        <p className="db-status">
          {callsignCount.toLocaleString()} callsigns loaded
        </p>
      )}

      {dbStatus === 'error' && <p className="db-error">Error: {dbError}</p>}
      ...
      <div className="keyboard-hints">
        <p>
          <kbd>Enter</kbd> or <kbd>F5</kbd> to start
        </p>
      </div>
    </div>
  );
