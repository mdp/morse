...
import {
  buildCallsignChallenge,
  type SegmentChallenge,
} from '../utils/segmentSplitter';
      ...
      <div className="game-content">
        <div className="current-call-section">
          ...
          <div className="callsign-area">
            ...
            <div className="speed-badge">{currentSpeedCpm} CPM</div>
          </div>
          ...
          {currentChallenge && !state.isPlaying && (
            <div className="mobile-choices">
              {currentChallenge.choices.map((choice, i) => {
                ...
                if (flashIndex === i && flashState === 'correct')
                  btnClass += ' correct';
                if (flashIndex === i && flashState === 'wrong')
                  btnClass += ' wrong';
                return (
                  <button
                    key={i}
          ...
          <div className="results-section">
            ...
            <div className="results-list">
              {[...state.results].reverse().map((result, i) => (
                <div
                  key={state.results.length - 1 - i}
                  className={`result-row ${result.correct ? 'correct' : 'incorrect'}`}
                >
