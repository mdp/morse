...
export function splitCallsign(call: string): string[] {
  ...
  if (lastDigitIdx === -1 || lastDigitIdx === upper.length - 1) {
    ...
    segments = [
      upper.slice(0, lastDigitIdx + 1),
      upper.slice(lastDigitIdx + 1),
    ];
  }
 ...
 */
export function generateSegmentChoices(
  segment: string,
  numChoices: number = 5
): SegmentChallenge {
  ...
  while (choices.size < numChoices && attempts < 100) {
    ...
    const indices = shuffle(
      Array.from({ length: chars.length }, (_, i) => i)
    ).slice(0, numSwaps);
    ...
    for (const idx of indices) {
      ...
      if (confusables.length > 0) {
        chars[idx] =
          confusables[Math.floor(Math.random() * confusables.length)];
      }
    }
