...
import {
  getCallsignCount,
  initCallsignDatabase,
} from './utils/callsignDatabase';
import { calculateStats } from './utils/scoring';

export default function App() {
  const { settings, setSettings, highScores, addHighScore, clearHighScores } =
    useStorage();
  const {
    state,
    startGame,
  ...
  useEffect(() => {
    if (state.phase === 'results' && state.results.length > 0) {
      ...
      const isNew =
        highScores.length === 0 || entry.score > highScores[0].score;
      addHighScore(entry);
