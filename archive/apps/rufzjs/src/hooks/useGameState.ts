...
import type { AttemptResult, GameState, Settings } from '../types';
...
function gameReducer(
  state: GameStateWithCallsigns,
  action: GameAction
): GameStateWithCallsigns {
  switch (action.type) {
    case 'START_GAME': {
    ...
    case 'SET_PLAYING':
      return {
        ...
        callsignStartTime: action.isPlaying
          ? Date.now()
          : state.callsignStartTime,
      };
    ...
    case 'SUBMIT_ANSWER': {
      ...
      if (state.settings.speedMode === 'adaptive') {
        ...
        if (isExact) {
          newSpeed =
            state.currentSpeed + Math.max(1, Math.round(speedAdjustment));
        } else {
          newSpeed = Math.max(
            5,
            state.currentSpeed - Math.max(1, Math.round(speedAdjustment))
