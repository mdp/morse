...
export default function AudioPlayer({
  ...
  const displayBars = useMemo(() => {
    if (!hasBars) return []
    const target = Math.max(16, Math.min(bars!.length, Math.floor(trackWidth / PX_PER_BAR)))
  ...
  return (
    <div className="border rounded-lg p-3 sm:p-4">
      ...
      <div className="flex items-center gap-3">
        ...
        >
          {playing ? <Pause className="size-5 sm:size-4" /> : <Play className="size-5 sm:size-4" />}
        </button>
