...
import { greedyDecode, cer } from './decode'
import { CHARS, NUM_CLASSES, BLANK_IDX, IDX_TO_CHAR } from './constants'
...
describe('greedyDecode', () => {
  it('collapses repeated frames into one character', () => {
    ...
    const r = greedyDecode(lp, 20, { entropyThreshold: 0, blankRatioThreshold: 0.99 })
  ...
  it('separates same character with blank between', () => {
    ...
    const lp = logProbsFromWinners(winners)
    const r = greedyDecode(lp, winners.length, { entropyThreshold: 0, blankRatioThreshold: 0.99 })
