...
This package lives in the [Morse monorepo](../../README.md) (Turborepo + Bun). From the repo root:

```sh
bun install
bunx turbo build --filter=react-morse-audio
```

`react-morse-audio` depends on the workspace `morse-audio` package, so Turbo builds
