 ...
 * ```
 */
export function useContestAudio(
  options: UseContestAudioOptions = {}
  ...
  // Initialize engine — qrn intentionally omitted from deps; changes handled by dedicated effect below
  // biome-ignore lint/correctness/useExhaustiveDependencies: qrn changes handled separately to avoid engine restarts
  useEffect(() => {
  ...
  const playStation = useCallback(
    async (stationOptions: PlayStationOptions): Promise<string> => {
      if (!engineRef.current) {
  ...
  const playSidetone = useCallback(
    async (sidetoneOptions: PlaySidetoneOptions): Promise<void> => {
      if (!engineRef.current) {
        throw new Error('Engine not initialized');
      }
      setIsSending(true);
