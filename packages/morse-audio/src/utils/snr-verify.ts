...
import { DEFAULT_REFERENCE_PEAK, generateCalibratedNoise } from './snr-mixing';
...
function makeKeyedSignal(
  text: string,
  wpm: number
): { samples: Float32Array; envelope: Float32Array } {
...
function measure(
  snrDb: number,
  signal: Float32Array,
  signalMask: Float32Array,
  noise: Float32Array
): Measurement {
  const signalGain = 10 ** (snrDb / 20);
  ...
  for (let i = 0; i < signal.length; i++)
    scaledSignal[i] = signal[i] * signalGain;
...
function main() {
  ...
  for (const snr of [-18, -12, -6, 0, 6, 12, 20, 30]) {
    const m = measure(
      snr,
      contSignal,
      new Float32Array(contSignal.length).fill(1),
      contNoise
    );
    console.log(
  ...
  const { samples: keyedSignal, envelope } = makeKeyedSignal(
    'EEEEEEEEEEEEEEEE',
    20
  );
