...
function generateRealisticStatic(
  ...
  for (let i = 0; i < length; i++) {
    ...
    const crackle = crackleSmooth.process(
      gaussianNoise(prng) * crackleEnvelope
    );
    ...
    const tone1 = Math.sin(
      2 * Math.PI * (toneFreq1 + toneDrift1 * time) * time
    );
    const tone2 = Math.sin(
      2 * Math.PI * (toneFreq2 + toneDrift2 * time) * time
    );
...
export function applyRadioEffects(
  ...
  // QRN settings
  const qrn = options.qrn;
  const hasQrn = !!qrn;
  const snr = qrn ? validateSnr(qrn.snr ?? DEFAULT_SNR) : 0;
  // Convert SNR (dB) to noise amplitude
  // Signal is ~0.8, so noise amplitude = 0.8 / 10^(snr/20)
  const noiseAmplitude = qrn ? 0.8 / 10 ** (snr / 20) : 0;

  // QSB settings
  const qsb = options.qsb;
  const hasQsb = !!qsb;
  const fadeDepth = qsb
    ? validateFadeDepth(qsb.depth ?? DEFAULT_FADE_DEPTH)
    : 0;
  const fadeRate = qsb ? validateFadeRate(qsb.rate ?? DEFAULT_FADE_RATE) : 0;
