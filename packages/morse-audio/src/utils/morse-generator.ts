...
import type { GeneratedMorseAudio, MorseGeneratorOptions } from '../types';
...
import { getMIMEType, getData as getRiffWaveData } from './riffwave';
 ...
 */
export function generateMorseAudio(
  options: MorseGeneratorOptions
): GeneratedMorseAudio {
  const {
    text,
    wpm: rawWpm,
  ...
  const dataUri = getDataURI(
    getRiffWaveData(samples, sampleRate),
    getMIMEType()
  );
