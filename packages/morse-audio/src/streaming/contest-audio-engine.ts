...
export class ContestAudioEngine implements IContestAudioEngine {
  ...
  private activeStations: Map<
    string,
    { source: AudioBufferSourceNode; gain: GainNode; info: ActiveStation }
  > = new Map();
  private activeSidetone: {
  ...
  constructor(options: ContestEngineOptions = {}) {
    ...
    this.centerFrequency = validateFrequency(
      options.centerFrequency ?? DEFAULT_FREQUENCY
    );
    this.sidetoneFrequency = validateFrequency(
      options.sidetoneFrequency ?? DEFAULT_FREQUENCY
    );
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      try {
        ...
        this.qrnWorkletNode = new AudioWorkletNode(
          this.audioContext,
          'qrn-processor'
        );
        ...
        console.warn(
          'AudioWorklet not supported, QRN will be disabled:',
          workletError
        );
  ...
  async playStation(options: PlayStationOptions): Promise<string> {
    if (!this.audioContext || !this.receiverGain || this.status !== 'running') {
    ...
    const fwpm =
      options.fwpm !== undefined ? validateFwpm(options.fwpm, wpm) : undefined;
    const frequencyOffset = validateFrequencyOffset(
      options.frequencyOffset ?? 0
    );
    ...
    const gain = 10 ** (signalStrength / 20);
  ...
  async playSidetone(options: PlaySidetoneOptions): Promise<void> {
    if (!this.audioContext || !this.sidetoneGain || this.status !== 'running') {
      throw new Error('Engine not running');
    }
    ...
    const fwpm =
      options.fwpm !== undefined ? validateFwpm(options.fwpm, wpm) : undefined;
    const frequency = validateFrequency(
      options.frequency ?? this.sidetoneFrequency
    );
  ...
  stopStation(id: string): void {
    ...
    if (station && this.audioContext) {
      ...
      station.gain.gain.exponentialRampToValueAtTime(
        0.001,
        currentTime + ContestAudioEngine.FADE_OUT_DURATION
      );
  ...
  stopSidetone(): void {
    if (this.activeSidetone && this.audioContext) {
      const currentTime = this.audioContext.currentTime;
      // Fade out to prevent click
      this.activeSidetone.gain.gain.setValueAtTime(
        this.activeSidetone.gain.gain.value,
        currentTime
      );
      this.activeSidetone.gain.gain.exponentialRampToValueAtTime(
        0.001,
        currentTime + ContestAudioEngine.FADE_OUT_DURATION
      );
      // Stop after fade-out completes
      try {
        this.activeSidetone.source.stop(
          currentTime + ContestAudioEngine.FADE_OUT_DURATION
        );
      } catch {
        // Ignore if already stopped
 ...
 */
export function createContestAudioEngine(
  options?: ContestEngineOptions
): IContestAudioEngine {
