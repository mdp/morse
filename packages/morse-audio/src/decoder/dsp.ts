...
import { butterworthBandpass, butterworthLowpass, sosfiltfilt } from './iir.js';
...
function computeMid40(audio: Float64Array, toneHz: number): Float64Array {
...
function computePhase50(audio: Float64Array, toneHz: number): Float64Array {
...
function computeSTFT2048(audio: Float64Array, toneHz: number): Float64Array {
  const winSize = 2048;
  const hop = 16;
  const nFFT = winSize;
...
function computeAutocorrV11(audio: Float64Array, toneHz: number): Float64Array {
...
export function computeV11Features(
  ...
  if (sampleRate !== SAMPLE_RATE) {
    throw new Error(
      `computeV11Features requires sampleRate=8000, got ${sampleRate}`
    );
  }
