...
import { getMIMEType, getData as getRiffWaveData } from '../utils/riffwave';
import { calculatePileupDuration, generateStationAudio } from './station-chain';
import type {
...
function generateQRN(
  ...
  const noiseAmplitude = 0.5 / 10 ** (snrDb / 20);
  ...
  for (let i = 0; i < length; i++) {
    ...
    if (i >= nextCrackle) {
      ...
      nextCrackle =
        i + Math.floor((-Math.log(prng() || 0.001) / crackleRate) * sampleRate);
    }
    crackleEnvelope *= 0.995;
    const crackle = crackleSmooth.process(
      gaussianRandom(prng) * crackleEnvelope
    );
...
function validateOptions(options: PileupGeneratorOptions): void {
  ...
  if (receiver.centerFrequency < 400 || receiver.centerFrequency > 1200) {
...
export function generatePileupAudio(
  ...
  const { stations, receiver, preDelay = 300, postDelay = 100 } = options;
 ...
 */
export function generatePileupSamples(options: PileupGeneratorOptions): {
