...
// biome-ignore lint/suspicious/noExplicitAny: dynamic config builder — fields vary by runtime options
...
async function main(): Promise<void> {
  ...
  for await (const line of rl) {
    ...
    if (command.cmd === 'generate_batch') {
      ...
      // biome-ignore lint/suspicious/noExplicitAny: union of success/error shapes per batch item
      const metadata: any[] = [];
      ...
      process.stdout.write(
        `${JSON.stringify({ status: 'ok', count: metadata.length, metadata })}\n`
      );
      continue;
    }
