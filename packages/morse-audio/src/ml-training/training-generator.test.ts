...
describe('TrainingSampleGenerator', () => {
  ...
  it('should convert to WAV buffer', () => {
    ...
    expect(String.fromCharCode(view[0], view[1], view[2], view[3])).toBe(
      'RIFF'
    );
    expect(String.fromCharCode(view[8], view[9], view[10], view[11])).toBe(
      'WAVE'
    );
  });
  ...
  it('should generate batch of samples', () => {
    ...
    const samples = generator.generateBatch(
      5,
      texts,
      DEFAULT_DISTRIBUTIONS,
      12345
...
describe('translateWithMetadata', () => {
  ...
  it('should track character timing boundaries', () => {
    ...
    expect(result.characters[0].endMs).toBeLessThanOrEqual(
      result.characters[1].startMs
    );
  });
  ...
  it('should include word gaps', () => {
    ...
    const wordGaps = result.elements.filter(
      (e) => e.elementType === 'word_gap'
    );
...
describe('Fist Model', () => {
  ...
  it('should apply jitter to timings', () => {
    ...
    for (let i = 0; i < result.timings.length; i++) {
      totalDiff += Math.abs(
        Math.abs(result.timings[i]) - Math.abs(translation.timings[i])
      );
    }
...
describe('Ionospheric Fading', () => {
  it('should apply amplitude modulation', () => {
    ...
    const faded = applyIonosphericFading(
      samples,
      {
        depth: 0.7,
        rate: 2.0,
...
describe('Multipath', () => {
  it('should add delayed copies', () => {
    ...
    const multipath = applyMultipath(
      samples,
      {
        paths: [
...
describe('AGC', () => {
  it('should level varying amplitude signal', () => {
    ...
    const inputRange =
      Math.max(...inputVariances) - Math.min(...inputVariances);
    const outputRange =
      Math.max(...outputVariances) - Math.min(...outputVariances);
...
describe('DEFAULT_DISTRIBUTIONS', () => {
  it('should have valid probability ranges', () => {
    expect(DEFAULT_DISTRIBUTIONS.wpmRange[0]).toBeLessThan(
      DEFAULT_DISTRIBUTIONS.wpmRange[1]
    );
    expect(DEFAULT_DISTRIBUTIONS.snrRange[0]).toBeLessThan(
      DEFAULT_DISTRIBUTIONS.snrRange[1]
    );
    expect(DEFAULT_DISTRIBUTIONS.frequencyRange[0]).toBeLessThan(
      DEFAULT_DISTRIBUTIONS.frequencyRange[1]
    );
  });

  it('should have valid fist distribution', () => {
    const sum = Object.values(DEFAULT_DISTRIBUTIONS.fistDistribution).reduce(
      (a, b) => a + (b ?? 0),
      0
    );
    expect(sum).toBeCloseTo(1.0, 5);
  ...
  it('should have probabilities between 0 and 1', () => {
    expect(
      DEFAULT_DISTRIBUTIONS.ionosphericFadingProbability
    ).toBeGreaterThanOrEqual(0);
    expect(
      DEFAULT_DISTRIBUTIONS.ionosphericFadingProbability
    ).toBeLessThanOrEqual(1);
    expect(DEFAULT_DISTRIBUTIONS.multipathProbability).toBeGreaterThanOrEqual(
      0
    );
